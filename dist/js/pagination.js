...
window.addEventListener('DOMContentLoaded', function () {
	...
	const paginationAppend = document.querySelector(".pagination-wrap") //! ТОТ СЕЛЕКТОР В КОТОРЫЙ ПОЛОЖИМ Пагинацию
	...
	class PaginationButton {
		constructor(totalPages, maxPagesVisible = 10, currentPage = 1) {
			...
			buttons.set(
				createAndSetupButton(`20`, 'end-page', disabled.end(), () => currentPage = totalPages),
			...
			this.render = (container = paginationAppend) => {
				container.prepend(paginationButtonContainer);
			};
	...
	const paginationButtons = new PaginationButton(20, 5); //! 23 количество страниц 5 вывод в обертку

	paginationButtons.render();

	paginationButtons.onChange(e => {
		console.log('-- страница', e.target.value)
	});
